...
\begin{table}[]
	...
	\resizebox{\linewidth}{!}{
		\begin{tabular}{lccccc}
			...
			&               & \ac{minADE} $\downarrow$ & \ac{minFDE} $\downarrow$ & \ac{minADE} $\downarrow$ & \ac{minFDE} $\downarrow$ \\ 
			\midrule
			$\dag$ Our Map-free Baseline (\ac{BSB}, No Hard-mining, Loss = \ac{NLL}) &  \textbf{105K}  & 1.79 & 4.01 & 0.81 & 1.56 \\
			LaneGCN  \cite{liang2020learning} & 3.7M & \textbf{1.35} & \textbf{2.97} & \textbf{0.71} & \textbf{1.08} \\
			WIMP \cite{khandelwal2020if} (w/o map, \ac{NLL} loss) & $> 25 M$ & \boldblue{1.41} & 6.38 & 1.07 & 1.61 \\
			WIMP \cite{khandelwal2020if} (w/o map, \ac{WTA} loss) & $> 25 M$ & 1.45 & 3.19 & 0.75 & \boldblue{1.14} \\
			\midrule
			$\dag$ \ac{BSB} + $C$=1 (oracle) &  \boldblue{277K}  & 1.62 & 3.56 & 0.79 & 1.42 \\
